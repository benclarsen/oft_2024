...
chart_data <- 
  read_delim("02_Data/deid/caselist_deid.csv", delim = ",") %>% filter(sex == "women", problem_type == "Injury", when_occurred != "Other" | is.na(when_occurred)) %>% filter(timeloss >0, subsequent_cat != "exacerbation") %>%
  mutate(" " = "All injuries") %>%
...
ggsave("04_Analysis/results/women/sankey_women.pdf", pl, width = 16, height = 9, units = "cm")
ggsave("04_Analysis/results/women/sankey_women.png", pl, device = "png",  width = 16, height = 9, units = "cm", dpi = 600)
